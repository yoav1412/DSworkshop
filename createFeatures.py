...
def create_summary_statistics(full_data,columns_to_aggregate, aggregation_functions):
    ...
    leftHandStats.columns = ["L_"+col+"_"+stat if col not in ("ID", "Hand") else col for col,stat in leftHandStats.columns]
    rightHandStats.columns = ["R_"+col+"_"+stat if col not in ("ID", "Hand") else col for col,stat in rightHandStats.columns]
    ...
    ll_stats.columns = ["LL_" + col + "_" + stat if col not in ("ID", "Direction") else col for col, stat in ll_stats.columns]
    lr_stats = lr_rl_ll_rr_stats[lr_rl_ll_rr_stats.Direction == "LR"]
    lr_stats.columns = ["LR_" + col + "_" + stat if col not in ("ID", "Direction") else col for col, stat in lr_stats.columns]
    rl_stats = lr_rl_ll_rr_stats[lr_rl_ll_rr_stats.Direction == "RL"]
    rl_stats.columns = ["RL_" + col + "_" + stat if col not in ("ID", "Direction") else col for col, stat in rl_stats.columns]
